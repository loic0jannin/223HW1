...
def plot_correlation_spread(data):
    # add the spread column
    data['spread'] = np.abs(data.best_offer - data.best_bid)

    # Compute the correlation
    correlation = data['volume'].corr(data['spread'])

    # Create a scatter plot
    plt.scatter(data['volume'], data['spread'])
    plt.xlabel('Volume')
    plt.ylabel('Spread')
    plt.title(f'Correlation between volume and spread: {correlation:.2f}')
    plt.show()

data = read_data("data/SPX-options.csv")

# plot the correlation
plot_correlation_spread(data)
